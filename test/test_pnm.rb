  ...
  def test_reader_open
    Dir.mktmpdir {|d|
      Dir.chdir(d) {
        ...
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ba.pbm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:ba") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ga.pgm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:ga") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pa.ppm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:pa") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("bb.pbm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:bb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("gb.pgm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:gb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pb.ppm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pnm:pb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })

        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pbm:ba") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pgm:ga") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ppm:pa") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pbm:bb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pgm:gb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ppm:pb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })

        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ba.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("ga.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pa.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("bb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("gb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb::Reader.open("pb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
      }
    }
  end
