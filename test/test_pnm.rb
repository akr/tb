  ...
  def test_reader_open
    Dir.mktmpdir {|d|
      Dir.chdir(d) {
        ...
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ba.pbm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:ba") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ga.pgm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:ga") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pa.ppm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:pa") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("bb.pbm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:bb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("gb.pgm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:gb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pb.ppm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pnm:pb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })

        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pbm:ba") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pgm:ga") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ppm:pa") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pbm:bb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pgm:gb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ppm:pb") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })

        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ba.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("ga.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pa.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("bb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("gb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
        assert_equal(["type", "x", "y", "component", "value"], Tb.open_reader("pb.pnm") {|r| header = nil; r.header_and_each(lambda {|h| header = h}) {}; header })
      }
    }
  end
